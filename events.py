...
from misc import *
from chess import *
from display.displayGame import (
    affect_mooves,
    affect_last_mooves,
    affect_draw_piece_mouse_cursor,
    increment_from_last_move,
    decrement_from_last_move,
    add_move_in_algebric_list,
)
import chess.engine
...
stockfish_settings = {"Threads": 1, "Hash": 16, "Skill Level": 20}
...
def clear_globals_events():
    global selected_piece, selected_piece_case, total_moves, counter_mooves, engine, is_stockfish_init, is_turn_IA, variant_engine
    selected_piece = None
        ...
        for line in file_path_ia.readlines():
            try:
                variant_engine = stockfish.Stockfish(
                    line, parameters=stockfish_variant_settings
                )
            except NameError:
                continue
...
def manage_events(
    ...
    is_opponent_bot=False,
    is_orientation_board_white=True,
    difficulty=20,
):
    """
    ...
    for event in events:
        ...
        if event.type == KEYDOWN:
            if event.key == K_ESCAPE:
                raise ChessException()
            if isinstance(board, chess.variant.CrazyhouseBoard) and any(
                [
                    check_pokets_non_empty(board, chess.Piece(type_p, board.turn))
                    for type_p in chess.PIECE_TYPES
                ]
            ):
                if event.key == K_q:
                    piece_on_mouse_cursor = Piece.from_symbol(
                        "Q" if board.turn else "q"
                    )
                    affect_draw_piece_mouse_cursor(
                        draw=True, piece=piece_on_mouse_cursor
                    )
                elif event.key == K_p:
                    piece_on_mouse_cursor = Piece.from_symbol(
                        "P" if board.turn else "p"
                    )
                    affect_draw_piece_mouse_cursor(
                        draw=True, piece=piece_on_mouse_cursor
                    )
            ...
            if (
                "piece_on_mouse_cursor" in globals()
                and piece_on_mouse_cursor is not None
            ):  # ch drop move
                pos = mouse.get_pos()
                ...
                if square is not None:
                    ...
                    pseudo_move = Move(
                        square, square, drop=piece_on_mouse_cursor.piece_type
                    )
                    if (
                        not piece_on_mouse
                        and check_pokets_non_empty(board, piece_on_mouse_cursor)
                        and is_drop_valid(pseudo_move, board)
                    ):
            ...
            try:
                if (
                    ...
                    and (0, 0)
                    <= mouse.get_pos()
                    <= (min(screen.get_size()), min(screen.get_size()))
                ...
                if mouse_buttons[0] and not is_turn_IA:  # Selection of a piece
                    if (
                        ...
                        if square is not None:
                            piece_on_mouse = board.piece_at(square)
                            if (
                                counter_mooves == len(total_moves)
                                and piece_on_mouse is None
                                and check_pokets_non_empty(board, piece_on_mouse_cursor)
                            ):
                                # do a drop moove ? not on a piece and if possible
                                is_promotion = is_move_a_promotion(
                                    selected_piece, Move(selected_piece_case, square)
                                )
                                pseudo_move = Move(
                                    square,
                                    square,
                                    drop=piece_on_mouse_cursor.piece_type,
                        ...
                        if case is not None:
                            ...
                            if selected_piece is not None and counter_mooves == len(
                                total_moves
                            ):  # do a moove ?
                                selected_piece_moves = get_piece_legal_moves(
                                    selected_piece, board, selected_piece_case
                                )
                                is_promotion = is_move_a_promotion(
                                    selected_piece, Move(selected_piece_case, case)
                                )
                                pseudo_move = Move(
                                    selected_piece_case,
                                    case,
                                ...
                                if is_move_in_mooves(pseudo_move, selected_piece_moves):
                                    if isinstance(board, chess.variant.CrazyhouseBoard):
                                        if board.is_capture(pseudo_move):
                                            if board.is_en_passant(pseudo_move):
                                                piece_taken = board.piece_at(
                                                    pseudo_move.to_square
                                                    + (
                                                        8
                                            ...
                                            board.pockets[selected_piece.color].add(
                                                piece_taken
                                            )
                                    add_move_in_algebric_list(pseudo_move, board)
                                    board.push(pseudo_move)
                                    if (
                                        ...
                                        if m is None:
                                            ...
                                            return
                                        variant_engine.make_moves_from_current_position(
                                            [pseudo_move.uci()]
                                        )
                                    is_turn_IA = is_opponent_bot
                                    total_moves.append(pseudo_move)
                            ...
                            selected_piece = None
                            if piece_on_mouse is not None and counter_mooves == len(
                                total_moves
                            ):
                                ...
                                affect_mooves(
                                    get_piece_legal_moves(piece_on_mouse, board, case)
                                )

...
def show(
    ...
    global MOOVES, last_move, imported_cairosvg, draw_piece_mouse_cursor, piece_mouse_cursor, from_index_draw_moves, to_index_draw_move
    ...
    if board.is_check():
        for square in board.attacks(last_move.to_square):
            if board.piece_at(square):
                ...
                if (piece.piece_type == chess.KING) and piece.color is board.turn:
                    king_square = square
    ...
    if imported_cairosvg:
        svg2png(
            bytestring=chess.svg.board(
                ...
                size=size_board,
                arrows=[
                    chess.svg.Arrow(chess.D4, chess.D4),
    ...
    draw_moves_board(
        screen, board, size_board, from_index_draw_moves, to_index_draw_move
    )


def draw_ch_pocket(
    screen: pygame.Surface, board: chess.variant.CrazyhouseBoard, board_size: int
):
    """
    ...
    cell_size = max(
        min(abs((width - 10 - board_size) // 10), abs(height // 10 - 10)), 1
    )
    pygame.draw.rect(
        screen,
        BLACK,
        rect=pygame.Rect((board_size, height // 2 - 1, width - 1, height - 1)),
        width=5,
    )
    ...
    texts = [
        str(board.pockets[chess.BLACK].count(piece_type=piece_t))
        for piece_t in [1, 4, 5, 2, 3]
    ] + [
        str(board.pockets[chess.WHITE].count(piece_type=piece_t))
        for piece_t in [1, 4, 5, 2, 3]
    ]

    images_white = [
        pygame.transform.scale(
            pygame.image.load("pictures/{0}".format(image)).convert_alpha(),
            (cell_size, cell_size),
        )
    ...
    crasyhouse_pocket = ButtonArray(
        ...
        images=images,
        texts=texts,
        textHAligns=["right" for _ in range(10)],
    )
    crasyhouse_pocket.draw()


def draw_moves_board(
    screen: pygame.Surface,
    board: chess.Board,
    board_size: int,
    from_index: int,
    ...
    pygame.draw.rect(
        screen,
        BLACK,
        rect=pygame.Rect((board_size, 0, width - 1, height // 2 - 4)),
        width=5,
    )
    ...
    bias = 0
    for index in range(
        min(from_index, len(board.move_stack)), min(to_index, len(board.move_stack))
    ):
...
def clear_globals_draw_game():
    ...
    """
    global last_move, MOOVES, algebraic_moves, draw_piece_mouse_cursor, piece_mouse_cursor, from_index_draw_moves, to_index_draw_move
    last_move = None
    MOOVES = []
    algebraic_moves = []
    draw_piece_mouse_cursor = False
...
def draw_piece_on_mouse_cursor(
    screen, board: chess.variant.CrazyhouseBoard, piece: chess.Piece, board_size: int
):
    """
    ...
    if board.pockets[piece.color].count(piece.piece_type) > 0:
        screen.blit(
            pygame.transform.scale(
                pygame.image.load(
                    "./pictures/{0}.png".format(map_pieces[piece.symbol()])
                ).convert_alpha(),
                (cell_size, cell_size),
            ),
            pos,
...
def draw_end_game(screen: pygame.Surface, outcome: chess.Outcome):
    ...
    pygame.draw.rect(
        screen,
        BLACK,
        rect=pygame.Rect((min(width, height) // 2, 0, width - 1, height // 2 - 4)),
        width=5,
    )
